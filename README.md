...
`import-wallet-pq` takes the key material from one of two mutually exclusive
options:

- **`--key-file <path>`** (preferred) — read the hex-encoded key material from a
  file, or from **stdin** when `<path>` is `-`. Surrounding whitespace/newlines are
  stripped, and an empty input is rejected. This keeps the secret out of the shell
  history and the process list (`ps`).
- **`--extended-private-key-hex <hex>`** (discouraged) — pass the hex directly on
  the command line. Functional but exposes the secret in shell history and in `ps`.

Passing both is rejected; passing neither is a usage error. Either way the value
must be a seed or persisted private key whose length matches the scheme (see the
REPL `ImportWalletPQ` table above).

`generate-pq-key` returns a JSON envelope whose `data` contains `scheme`, `seed`,
`private_key`, `public_key`, `address`, `private_key_persisted_form`, and a
scheme-aware `warning` (for `FN_DSA_512` it flags that the seed only reproduces the
key on the same architecture/JVM). `register-wallet-pq` and `import-wallet-pq`
return the keystore filename and the derived address, exactly like the
standard-CLI `register-wallet` / `import-wallet` commands.
